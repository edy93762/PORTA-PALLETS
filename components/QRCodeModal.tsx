...
import { Loader2, FileDown, X, PackageCheck, PackagePlus, Box } from 'lucide-react';
...
export const QRCodeModal: React.FC<QRCodeModalProps> = ({ position, onClose, item, onManage }) => {
  ...
  const levelLabelsPP = ['A', 'B', 'C', 'D', 'E'];
  const levelLabelsPR = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H'];
  
  let labelText = '';
  let codeValue = '';

  if (position.rack === 'GAIOLA') {
      labelText = `GAIOLA ${position.pos}`;
      codeValue = `GAIOLA ${position.pos}`;
  } else if (['1','2','3','4'].includes(position.rack)) {
      // Prateleira
      const levelChar = levelLabelsPR[position.level - 1];
      labelText = `PRAT ${position.rack} - ${levelChar}`;
      // Código interno para scanner: PR-{RACK}-G-1-L-{LEVEL}
      codeValue = `PR-${position.rack}-G-1-L-${levelChar}`; 
  } else {
      // Porta-Pallet (A-D)
      const levelChar = levelLabelsPP[position.level - 1];
      // Formato Visual: A 4 A (Rua Posição Altura com espaços)
      labelText = `${position.rack} ${position.pos} ${levelChar}`;
      // Valor do QR Code com espaços
      codeValue = `${position.rack} ${position.pos} ${levelChar}`;
  }
  ...
  const handleDownloadSingle = async () => {
    ...
    try {
      ...
      doc.save(`Etiqueta_${labelText}.pdf`);
    } catch (e) {
      console.error(e);
    ...
    <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-md flex items-center justify-center z-[9000] p-4">
      <div className="bg-white rounded-[2.5rem] shadow-2xl max-w-sm w-full overflow-hidden animate-in zoom-in-95 duration-200 relative">
        ...
        <div className={`h-24 w-full flex items-center justify-center relative ${isOccupied ? 'bg-rose-500' : 'bg-emerald-500'}`}>
            ...
            <h2 className="text-white font-black text-2xl italic tracking-widest uppercase opacity-90 text-center px-4 leading-tight">{labelText}</h2>
        </div>
        ...
        <div className="p-8 flex flex-col items-center -mt-6">
            {isOccupied ? (
                <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-6 w-full mb-6 relative z-10">
                    ...
                    <div className="flex justify-between items-center pt-3 border-t border-slate-100">
                        <div><span className="text-[9px] font-bold text-slate-400 uppercase block">Quantidade</span><span className="text-2xl font-black text-indigo-600">{item.quantity} <span className="text-xs text-slate-400">un</span></span></div>
                        <div className="text-right"><span className="text-[9px] font-bold text-slate-400 uppercase block">ID / SKU</span><span className="text-sm font-black text-slate-600 bg-slate-100 px-2 py-1 rounded-md">{item.productId}</span></div>
                    </div>
                </div>
            ) : (
                <div className="bg-white rounded-2xl shadow-md border border-slate-100 p-4 w-full mb-6 relative z-10 text-center">
                    <span className="text-emerald-500 font-black uppercase text-sm flex items-center justify-center gap-2"><PackageCheck size={18}/> Local Livre</span>
                </div>
            )}
            ...
            <button onClick={handleDownloadSingle} disabled={loading} className="w-full bg-slate-800 hover:bg-slate-900 text-white font-black py-4 rounded-xl transition-all shadow-lg text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 active:scale-95">
