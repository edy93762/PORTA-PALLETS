...
// Configurado para os 4 porta-paletes (A, B, C, D)
...
const App: React.FC = () => {
  ...
  const loadInitialData = async () => {
    ...
    } catch (e) { showFeedback('error', 'Falha na conexão com servidor.'); }
  };
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    } catch (e) { showFeedback('error', 'Erro no banco de dados.'); }
  ...
  const handlePositionClick = (rack: RackId, level: number, pos: number) => {
    ...
    if (blocked) { showFeedback('error', 'Espaço ocupado por palete duplo adjacente.'); return; }
  ...
  const handleSavePosition = async (e: React.FormEvent) => {
    ...
    if (!selectedPosition.productId?.trim() || !selectedPosition.productName?.trim() || selectedPosition.quantity === undefined) {
      showFeedback('error', 'Preencha SKU, Nome e Quantidade!'); return;
    }
    ...
    } catch (err) { showFeedback('error', 'Falha ao salvar no banco.'); }
  ...
  const handleProcessExit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setExitQuantity('');
    } catch (err) { showFeedback('error', 'Falha ao processar baixa.'); }
    finally { setIsProcessingAction(false); }
  };
  ...
  const handleSaveUser = async (e: React.FormEvent) => {
    ...
    } catch (e) { showFeedback('error', 'Erro ao salvar usuário.'); }
    finally { setIsProcessingAction(false); }
  };
  ...
  const handleSaveMaster = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showFeedback('success', 'SKU cadastrado com sucesso!');
    } catch (e) { showFeedback('error', 'Erro ao cadastrar SKU.'); }
    finally { setIsProcessingAction(false); }
  };
  ...
  const handlePrintBatch = async () => {
    ...
    try {
      ...
      showFeedback('success', 'Arquivo PDF de etiquetas gerado!');
      setIsPrintMenuOpen(false);
    } catch (e) { showFeedback('error', 'Erro ao gerar PDF.'); }
    finally { setIsProcessingAction(false); }
  };
  ...
  const stats = useMemo(() => {
    ...
    return { total: totalVagas, occupied: vagasOcupadas, free: totalVagas - vagasOcupadas, rate: totalVagas > 0 ? Math.round((vagasOcupadas / totalVagas) * 100) : 0 };
  }, [inventory]);
      ...
      <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-6">
        <div className="w-full max-w-sm text-center">
          ...
          <form onSubmit={handleLogin} className="bg-white/5 border border-white/10 p-8 rounded-[3rem] space-y-4">
            <input type="text" placeholder="LOGIN" className="w-full bg-white/5 border-2 border-white/10 p-4 rounded-2xl text-white font-bold uppercase" value={loginUsername} onChange={e => setLoginUsername(e.target.value.toLowerCase())} />
            <input type="password" placeholder="SENHA" className="w-full bg-white/5 border-2 border-white/10 p-4 rounded-2xl text-white font-bold" value={loginPassword} onChange={e => setLoginPassword(e.target.value)} />
            <button disabled={isLoggingIn} type="submit" className="w-full bg-indigo-600 text-white p-5 rounded-2xl font-black uppercase">
              {isLoggingIn ? 'Autenticando...' : 'Acessar'}
            </button>
            {loginError && <p className="text-rose-500 font-black text-xs uppercase">Login Inválido</p>}
          </form>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col pb-20 lg:pb-0 overflow-hidden relative">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-[1000] px-6 py-4 flex justify-between items-center shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-base font-black italic tracking-tighter uppercase leading-none hidden sm:block">ALMOX PRO</h1>
        </div>
        <div className="flex items-center gap-2">
          {highlightProductId && (
            <button onClick={() => setHighlightProductId(null)} className="p-3 bg-amber-50 text-amber-600 rounded-xl flex items-center gap-2 text-[10px] font-black uppercase shadow-inner">
              <Eraser size={16}/> Limpar
            </button>
          )}
          <button onClick={() => setIsSKUSearchOpen(true)} className="p-3 bg-indigo-50 text-indigo-600 rounded-xl flex items-center gap-2 text-[10px] font-black uppercase">
             <SearchCode size={18}/> Soma por ID
          </button>
          <button onClick={() => setViewMode(viewMode === '2d' ? '3d' : '2d')} className="p-3 bg-slate-100 text-slate-600 rounded-xl text-[10px] font-black uppercase">
             {viewMode === '2d' ? '3D' : '2D'}
          </button>
      ...
      <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
        ...
        <aside className="hidden lg:flex w-72 bg-white border-r border-slate-200 p-6 flex-col gap-3 h-[calc(100vh-65px)] overflow-y-auto no-scrollbar">
           <button onClick={() => setIsScannerOpen(true)} className="flex items-center gap-3 p-4 bg-indigo-600 text-white rounded-2xl font-black uppercase text-[11px] shadow-lg"><ScanLine size={18}/> Scanner Baixa</button>
           <button onClick={() => setIsManualExitModalOpen(true)} className="flex items-center gap-3 p-4 bg-rose-50 text-rose-600 rounded-2xl font-black uppercase text-[11px] border border-rose-100"><Navigation size={18}/> Saída Manual</button>
           <div className="h-px bg-slate-100 my-2"></div>
           <button onClick={() => setIsMasterMenuOpen(true)} className="flex items-center gap-3 p-4 hover:bg-slate-50 text-slate-600 rounded-2xl font-black uppercase text-[11px]"><ClipboardList size={18}/> Cadastro SKU</button>
           ...
           {currentUser.role === 'admin' && (
             <button onClick={async () => { const users = await fetchUsersFromDB(FIXED_DB_STRING); setUsersList(users); setIsUsersMenuOpen(true); }} className="flex items-center gap-3 p-4 bg-emerald-50 text-emerald-600 rounded-2xl font-black uppercase text-[11px] border border-emerald-100"><Users size={18}/> Usuários</button>
           )}
        </aside>
        ...
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-xl z-[6000] flex items-center justify-center p-6" onClick={() => setIsMasterMenuOpen(false)}>
           <div className="bg-white w-full max-w-md rounded-[3rem] shadow-2xl p-8 animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
              <header className="flex justify-between items-center mb-6">
                 <h3 className="font-black text-xl uppercase italic text-indigo-600">Cadastro de Produto</h3>
              ...
              <form onSubmit={handleSaveMaster} className="space-y-4">
                 ...
                 <button type="submit" className="w-full bg-indigo-600 text-white p-4 rounded-xl font-black uppercase text-xs flex items-center justify-center gap-2">
                    <Save size={16}/> Salvar Cadastro
                 </button>
              </form>
              <div className="mt-6 border-t pt-4 max-h-48 overflow-y-auto no-scrollbar">
                 <p className="text-[9px] font-black uppercase text-slate-400 mb-2">Itens em Base:</p>
                 {masterProducts.map(m => (
                    <div key={m.productId} className="flex justify-between items-center p-3 bg-slate-50 rounded-xl mb-2 text-[10px]">
                       <div><span className="font-black">{m.productId}</span> - <span className="opacity-60">{m.productName}</span></div>
           ...
           <div className="bg-white w-full max-w-sm rounded-[3rem] p-8 shadow-2xl" onClick={e => e.stopPropagation()}>
              ...
              <div className="space-y-4">
                 <div className="grid grid-cols-4 gap-2">
                    {RACKS.map(r => (
                       <button key={r} onClick={() => setPrintFilter({...printFilter, rack: r})} className={`p-3 rounded-xl font-black text-xs ${printFilter.rack === r ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-50'}`}>{r}</button>
                    ))}
                 </div>
                 ...
                 <button onClick={handlePrintBatch} disabled={isProcessingAction} className="w-full bg-indigo-600 text-white p-5 rounded-2xl font-black uppercase text-xs flex items-center justify-center gap-2 shadow-xl">
        ...
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-xl z-[6000] flex items-center justify-center p-4" onClick={() => setIsSKUSearchOpen(false)}>
           <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl flex flex-col overflow-hidden h-[85vh]" onClick={e => e.stopPropagation()}>
             <header className="p-6 bg-indigo-600 text-white flex justify-between items-center">
                <div className="flex items-center gap-3"><SearchCode size={24} /> <h3 className="font-black text-lg uppercase italic">Consulta de Estoque Total</h3></div>
                <button onClick={() => { setIsSKUSearchOpen(false); setSkuSearchQuery(''); }} className="p-2 bg-white/10 rounded-xl"><X size={20}/></button>
             </header>
             <div className="p-6 border-b border-slate-100">
                <input type="text" placeholder="DIGITE O SKU ID..." className="w-full p-5 bg-slate-50 rounded-2xl font-black outline-none border-4 border-transparent focus:border-indigo-500 shadow-inner uppercase text-center" value={skuSearchQuery} onChange={e => setSkuSearchQuery(e.target.value)} autoFocus />
             </div>
             <div className="flex-1 overflow-y-auto p-6 space-y-4 no-scrollbar">
                {skuSearchQuery.trim() !== '' ? (
                  <>
                    <div className="bg-amber-500 p-8 rounded-[2.5rem] text-white flex justify-between items-center shadow-lg animate-in zoom-in-95">
                       <div>
                          <p className="text-[10px] font-black uppercase opacity-80 mb-1">Unidades Totais no Sistema</p>
                          <h4 className="text-4xl font-black italic">{skuTotalQuantity} UN</h4>
                       </div>
                       <TrendingUp size={40} className="opacity-40" />
                    </div>
                    <div className="grid gap-3 pt-4">
                       <p className="text-[9px] font-black uppercase text-slate-400">Localizações de Paletes:</p>
                       {skuSearchResults.map(item => (
                          <div key={item.id} className="bg-white p-4 rounded-2xl border border-slate-200 flex items-center justify-between shadow-sm">
                             ...
                             <div className="flex items-center gap-2">
                                <button onClick={() => { setHighlightProductId(item.productId!); setIsSKUSearchOpen(false); showFeedback('success', `Destaque para ${item.productId}`); }} className="p-2 bg-amber-100 text-amber-600 rounded-lg"><Focus size={16}/></button>
                ...
                ) : <div className="py-20 text-center opacity-20 font-black italic uppercase">Insira um SKU para calcular</div>}
             </div>
           </div>
        </div>
      )}
      ...
      {isUsersMenuOpen && (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-xl z-[6000] flex items-center justify-center p-6" onClick={() => setIsUsersMenuOpen(false)}>
          <div className="bg-white w-full max-w-md rounded-[3rem] p-8 shadow-2xl" onClick={e => e.stopPropagation()}>
             <header className="flex justify-between items-center mb-6">
                <h3 className="font-black text-xl uppercase italic text-emerald-600">Gestão de Usuários</h3>
             ...
             <form onSubmit={handleSaveUser} className="space-y-3 mb-6 bg-emerald-50 p-6 rounded-3xl">
                <input type="text" placeholder="NOME DE USUÁRIO" className="w-full p-3 bg-white rounded-xl font-black uppercase text-xs" value={newUser.username} onChange={e => setNewUser({...newUser, username: e.target.value.toLowerCase()})} />
                <input type="password" placeholder="SENHA" className="w-full p-3 bg-white rounded-xl font-black text-xs" value={newUser.password || ''} onChange={e => setNewUser({...newUser, password: e.target.value})} />
                <button type="submit" className="w-full bg-emerald-600 text-white font-black p-4 rounded-xl uppercase text-[10px] shadow-lg">Registrar Novo Usuário</button>
             </form>
             <div className="max-h-56 overflow-y-auto no-scrollbar space-y-2">
                <p className="text-[8px] font-black uppercase text-slate-400">Usuários Ativos:</p>
                {usersList.map(u => (
                  <div key={u.username} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl text-[11px] font-bold uppercase group hover:bg-white border border-transparent hover:border-emerald-100 transition-all">
                     <div className="flex items-center gap-3">
                        <User size={16} className="text-emerald-500 opacity-50 group-hover:opacity-100"/>
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-xl z-[6000] flex items-center justify-center p-6" onClick={() => setSelectedPosition(null)}>
           <div className="bg-white rounded-[3rem] w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
             ...
             <form onSubmit={handleSavePosition} className="p-6 space-y-4">
                <input type="text" placeholder="ID SKU" className="w-full p-4 bg-slate-50 rounded-xl font-black uppercase" value={selectedPosition.productId || ''} onChange={e => {
                ...
                <input type="text" placeholder="NOME DO ITEM" className="w-full p-4 bg-slate-50 rounded-xl font-black uppercase" value={selectedPosition.productName || ''} onChange={e => setSelectedPosition({...selectedPosition, productName: e.target.value.toUpperCase()})} />
                <div className="grid grid-cols-2 gap-3">
                  <input type="number" placeholder="QUANTIDADE" className="w-full p-4 bg-slate-50 rounded-xl font-black" value={selectedPosition.quantity || ''} onChange={e => setSelectedPosition({...selectedPosition, quantity: parseInt(e.target.value) || 0})} />
                ...
                <div className="flex gap-2 pt-2">
                  <button type="submit" className="flex-1 bg-indigo-600 text-white p-4 rounded-2xl font-black uppercase text-xs shadow-lg">{isProcessingAction ? 'Salvando...' : 'Armazenar'}</button>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[7000] flex justify-end" onClick={() => setIsMobileMenuOpen(false)}>
           <div className="w-[80%] max-w-xs h-full bg-white shadow-2xl flex flex-col p-8 animate-in slide-in-from-right" onClick={e => e.stopPropagation()}>
              ...
              <div className="space-y-3">
                 ...
                 <button onClick={() => {setIsMobileMenuOpen(false); setIsMasterMenuOpen(true);}} className="w-full flex items-center gap-4 p-4 rounded-2xl bg-slate-50 text-slate-700 font-black uppercase text-xs"><ClipboardList size={18}/> Cadastro SKU</button>
      ...
      {scannedPosition && (
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl z-[8000] flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-sm rounded-[3rem] p-8 animate-in zoom-in-95 shadow-2xl">
            <header className="flex justify-between items-center mb-6"><h3 className="font-black text-xl text-rose-600 uppercase">Retirar Item</h3><button onClick={() => setScannedPosition(null)} className="p-2"><X/></button></header>
      ...
      {isManualExitModalOpen && (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-xl z-[6000] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-sm rounded-[3rem] p-8 shadow-2xl">
             <header className="flex justify-between items-center mb-6"><h3 className="font-black text-xl text-rose-600 uppercase">Localizar Local</h3><button onClick={() => setIsManualExitModalOpen(false)}><X/></button></header>
